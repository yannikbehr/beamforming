...
"""
Plane wave beamforming for the horizontal components.
"""
...
def response(datdir, filesN, filesE, nprep=False, nbeam=False, doplot=True):
    """
...
def sort_list(files):
    ...
    for _f in files:
        ...
        stat, chan, format = fn.split('.')
        if stat not in file_pairs.keys():
            file_pairs[stat] = {}
        file_pairs[stat][chan] = _f
    for _st in file_pairs.keys():
        if len(file_pairs[_st].keys()) >= 2:
            for _k in file_pairs[_st].keys():
                if _k.endswith('N'):
                    filesN.append(file_pairs[_st][_k])
                if _k.endswith('E'):
                    filesE.append(file_pairs[_st][_k])
    return filesN, filesE

def main(datdir, filesN, filesE, nprep=False, nbeam=False, doplot=True,
         save=False, nostat=20):
...
def proc_main():
    ...
    flist.pop()
    filesN, filesE = sort_list(flist)

    if opts.aresp:
        datdir = args[0]
        response(datdir, filesN, filesE, nbeam=opts.beam, nprep=opts.data, doplot=opts.plot)
    else:
        datdir = args[0]
        main(datdir, filesN, filesE, nbeam=opts.beam, nprep=opts.data, doplot=opts.plot,
